...
int init_egl(const device_opts_t* p_opts, device_info_t* p_info)
{
  ...
  // log_info("Using display %p with EGL version %d.%d", g_egl_data.display, g_egl_data.major_version, g_egl_data.minor_version);
  // log_info("EGL Version \"%s\"", eglQueryString(g_egl_data.display, EGL_VERSION));
  // log_info("EGL Vendor \"%s\"", eglQueryString(g_egl_data.display, EGL_VENDOR));
  // log_info("EGL Extensions \"%s\"", eglQueryString(g_egl_data.display, EGL_EXTENSIONS));
